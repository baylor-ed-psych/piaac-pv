...
%macro PIAAC_TOOL(method= ,table= , wgt=SPFWT0 , rwgt=SPFWT , nrep=80 , nb_pv=1 , dvar= , dvarpv=0 , ivar= ,ivarpv= , 
				  byvar= ,byvarpv= ,
				  vemethod=VEMETHOD,
				  clvar= ,clvarpv= ,refclvarpv= ,
				  event=1, percentile= , R2= , out_R2=R2, probit=0,
				  predict=0, table_pred=pred,logintercept=no,
				  where_cond= ,ficout= ,path_out= , excel_sheet=sas_output, dbms=);
